    ...
    def generate_pystring_constants(self, text_strings: list, byte_strings: list):
        ...
        for i, (is_interned, cname, text) in enumerate(text_strings):
            ...
            if first_interned == -1 and is_interned:
                first_interned = i
            defines.putln(
                f"#define {cname} {Naming.stringtab_cname}[{i}]"
            )
    
        stringtab_bytes_start: cython.Py_ssize_t = len(text_strings)
        for i, (_, cname, text) in enumerate(byte_strings, start=stringtab_bytes_start):
            value = text.byteencode() if text.encoding else text.utf8encode()
            bytes_values.append(value)
            defines.putln(
                f"#define {cname} {Naming.stringtab_cname}[{i}]"
            )
    
        lengths = list(map(len, bytes_values))
        total_length = sum(lengths)
        plain_bytes = b"".join(bytes_values)
        key_len = 32
        key = secrets.token_bytes(key_len)
        key_index = list(range(key_len))
        secrets.SystemRandom().shuffle(key_index)
        stored_key = bytes(key[i] for i in key_index)
    
        encrypted_bytes = bytes(
            value ^ key[pos % key_len]
            for pos, value in enumerate(plain_bytes)
        )
    
        def c_array(data: bytes) -> str:
            return ", ".join(str(value) for value in data)
    
        def c_index_array(values: list[int]) -> str:
            return ", ".join(str(value) for value in values)
    
        if lengths:
            max_length_bits = max(lengths).bit_length()
        else:
            max_length_bits = 1
    
        w.putln("{")
        w.putln(
            f"const unsigned char bytes[{key_len}] = "
            f"{{{c_array(stored_key)}}};"
        )
        w.putln(
            f"const unsigned char index[{key_len}] = "
            f"{{{c_index_array(key_index)}}};"
        )
        w.putln(
            f"unsigned char pyx_runtime_key[{key_len}];"
        )
        w.putln(
            f"for (Py_ssize_t i = 0; i < {key_len}; i++) "
            "pyx_runtime_key[index[i]] = bytes[i];"
        )
    
    
        w.putln(
            "const struct { "
            f"const unsigned int length: {max_length_bits}; "
            "} pyx_index[] = "
            f"{{{','.join(['{%d}' % length for length in lengths])}}};"
        )
        w.putln(
            f"const unsigned char pyx_encrypted_bytes[{max(1, total_length)}] = "
            f"{{{c_array(encrypted_bytes) if encrypted_bytes else '0'}}};"
        )
        w.putln(f"unsigned char pyx_plain_bytes[{max(1, total_length)}];")
        w.putln(
            f"for (Py_ssize_t i = 0; i < {total_length}; i++) "
            "pyx_plain_bytes[i] = "
            "pyx_encrypted_bytes[i] ^ pyx_runtime_key[i % "
            f"{key_len}];"
        )
    
        w.putln(
            f"PyObject **stringtab = "
            f"{w.name_in_main_c_code_module_state(Naming.stringtab_cname)};"
        )
        w.putln("PyObject *data = NULL;")
        w.putln("Py_ssize_t pos = 0;")
    
        if stringtab_bytes_start > 0:
            w.putln(
                f"for ({'int' if stringtab_bytes_start < 2**15 else 'Py_ssize_t'} "
                f"i = 0; i < {stringtab_bytes_start}; i++) {{"
            )
            w.putln("Py_ssize_t bytes_length = pyx_index[i].length;")
            w.putln(
                "PyObject *string = PyUnicode_DecodeUTF8(" 
                "(const char *)(pyx_plain_bytes + pos), bytes_length, NULL);"
            )
    
            if first_interned >= 0:
                w.putln(
                    f"if (likely(string) && i >= {first_interned}) "
        ...
        if stringtab_bytes_start < len(lengths):
            w.putln(
                f"for ({'int' if len(lengths) < 2**15 else 'Py_ssize_t'} "
                f"i = {stringtab_bytes_start}; i < {len(lengths)}; i++) {{"
            )
            w.putln("Py_ssize_t bytes_length = pyx_index[i].length;")
            w.putln(
                "PyObject *string = PyBytes_FromStringAndSize(" 
                "(const char *)(pyx_plain_bytes + pos), bytes_length);"
            )
            w.putln("stringtab[i] = string;")
            w.putln("pos += bytes_length;")
            w.putln("if (unlikely(!string)) {")
        ...
        w.putln("Py_XDECREF(data);")
        w.putln(
            f"for (Py_ssize_t i = 0; i < {len(lengths)}; i++) {{"
        )
        ...
        if stringtab_bytes_start < len(lengths):
            self.immortalize_constants(
                f"stringtab + {stringtab_bytes_start}",
                len(lengths) - stringtab_bytes_start,
                w,
            )
    
        w.putln(
            f"for (Py_ssize_t i = 0; i < {total_length}; i++) pyx_plain_bytes[i] = 0;"
        )
        w.putln(
            f"for (Py_ssize_t i = 0; i < {key_len}; i++) pyx_runtime_key[i] = 0;"
        )
        w.putln("}")
